...
Prefer factory functions that accept overrides and return complete objects (`createUser(overrides)`). Seed test state through APIs, tasks, or direct DB helpers before visiting the UInever via slow UI interactions. UI is for validation only, not setup.
...
- Faker generates unique valuesno collisions in parallel tests
...
**Context**: When tests need data setup, always use API calls or database tasksnever UI navigation. Wrap factory usage with seeding utilities for clean test setup.
