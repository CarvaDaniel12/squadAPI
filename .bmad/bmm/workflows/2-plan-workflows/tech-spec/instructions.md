...
<check if="status file found">
  ...
  <check if="tech-spec status is file path (already completed)">
    <output> Tech-spec already completed: {{tech-spec status}}</output>
  ...
  <check if="tech-spec is not the next expected workflow">
    <output> Next expected workflow: {{next_workflow}}. Tech-spec is out of sequence.</output>
...
<ask>Fix validation issues? (yes/no)</ask>
  ...
  <check if="no">
    <output> Proceeding with warnings. Issues should be addressed manually.</output>
  </check>
</check>
