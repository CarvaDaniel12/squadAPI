"""Simplified Ollama integration test without Unicode issues."""
import asyncio
import sys
            ...
            async with session.get(
                ...
                if resp.status == 200:
                    ...
                    if any("qwen3" in name for name in model_names):
                        print("   OK: qwen3 model found!")
                        return True
                    elif any("llama3.2" in name for name in model_names):
                        print("   OK: llama3.2 model found (alternative)")
                        return True
                    elif any("phi3" in name for name in model_names):
                        print("   OK: phi3 model found (alternative)")
                        return True
                    else:
                        print("   WARNING: Recommended model not found")
                        print("   HINT: Run: ollama pull qwen3:8b")
                        return False
                else:
                    print(f"   ERROR: Ollama returned HTTP {resp.status}")
                    return False
    except asyncio.TimeoutError:
        print("   ERROR: Ollama not responding (timeout)")
        print("   HINT: Is Ollama running? Try: ollama serve")
        return False
    except Exception as exc:
        print(f"   ERROR: Error connecting to Ollama: {exc}")
        print("   HINT: Install Ollama from https://ollama.ai")
        return False
...
async def test_prompt_optimizer():
    ...
    print("\n3. Testing LocalPromptOptimizer...")
    ...
    print("   INFO: Synthesizing 3 specialist outputs...")
    ...
    if synthesized:
        ...
        print("   ERROR: Synthesis failed (returned None)")
        return False
...
async def main():
    ...
    if results[0][1]:  # Only if Ollama is running
        ...
        print("\nWARNING: Skipping remaining tests (Ollama not available)")
    ...
    if all(r[1] for r in results):  # Only if previous tests passed
        ...
        print("\nWARNING: Skipping prompt optimizer test")
    ...
    if passed_count == total_count:
        print("\nSUCCESS: All tests passed! Ollama integration is ready.")
        return 0
    else:
